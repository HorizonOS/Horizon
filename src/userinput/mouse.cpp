...
void ProcessMousePacket(){
    if (!MousePacketReady) return;
        ...
        if (MousePacket[0] & PS2Leftbutton){
            ...
            // GlobalRenderer->Colour = 0xffffff;
            GlobalRenderer->PutPix(MousePosition.X , MousePosition.Y , 0xffffff);
            // GlobalRenderer->Colour = colour;        
        }
        if (MousePacket[0] & PS2Middlebutton){
            // uint32_t colour = GlobalRenderer->Colour;
            // GlobalRenderer->Colour = 0xffff00;
            GlobalRenderer->PutPix(MousePosition.X , MousePosition.Y , 0xffff00);
            // GlobalRenderer->Colour = colour;
        }
        if (MousePacket[0] & PS2Rightbutton){
            // uint32_t colour = GlobalRenderer->Colour;
            // GlobalRenderer->Colour = 0x0000ff00;
            GlobalRenderer->PutPix(MousePosition.X , MousePosition.Y , 0x0000ff00);
            // GlobalRenderer->Colour = colour;
        }
